...
public class AdminController {
	...
	//예약내역 관리페이지
	@RequestMapping(value="/adminPage_Reserve_List.do", method = RequestMethod.GET)
	public String adminPage_Reserve_List(Model model,SearchVO searchVO) {
		
		//페이지네이션(검색어포함)
		PageVO pageVO = new PageVO(searchVO , adminService.reserve_Total(searchVO)); 
		List<ReserveVO> reserveList = adminService.reserveList(searchVO);
		
		model.addAttribute("reserveList", reserveList );
		model.addAttribute("pageVO", pageVO );
		
		return "adminPage/adminPage_Reserve_List";	
	}
}
